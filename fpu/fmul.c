...
long long int fmul(long long int i1,long long int i2){
   ...
   wire y = (1-bitand(e1) && 1-bitand(e2) && 1-bitor(ey) && bitand(extract(mye,46,23))) ? concat3(sy,init(8,1),init(23,0)) : // 繰り上がりで指数部が1，仮数部が0になるとき
            (1-bitand(e1) && 1-bitand(e2) && (extract(ey,9,9).val || 1-bitor(ey))) ? concat(sy,init(31,0)) :
            (1-bitand(e1) && 1-bitand(e2) && extract(ey,8,8).val) ? concat3(sy,init(8,255),init(23,0)) : concat3(sy,extract(ey,7,0),my); // overflowしたら符号を合わせて無限にする
